...
# This version creates a fingerprint for each event and compares the two fingerprints
...
class fingerprint:
    ...
    def similarity(self, fp_text):
	sim = 0.0
        for i in xrange(len(fp_text)):
            if self.topk.has_key(fp_text[i]):
                j = self.topk[ fp_text[i] ]
                sim += self.ff.value( max(i,j) ) 
        return float(sim)
...
class Classifier:
    ...
    def createmodel(self, K):
        ...
        for t in self.topics:
            topk = [ (w, f * itf(self.tf[w], len(self.topics)) ) for (w,f) in self.fp[t].freqwords(K) ]
            self.fp[t].setfp(topk, ff)
            #print >> sys.stderr, t, sorted(topk, key=operator.itemgetter(1), reverse=True)
	...
	''' Assumes that "words" is a list of words '''
        ...
        # Fingerprint da empresa
        wc = collections.Counter(words)
        topk=[]
        for w in words:
            if self.tf.has_key(w):
               topk.append( (w, wc[w] * itf(self.tf[w] , len(self.topics)) ) )
            else:
               topk.append( (w, wc[w]) )
        fp_text=[ w for (w,f) in sorted(topk,key=operator.itemgetter(1),reverse=True) ]

	scores = [ (t, self.fp[t].similarity( fp_text ) ) for t in self.topics ] 
